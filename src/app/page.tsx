...
import { AgentDropdown } from "@/features/agents/components/AgentDropdown";
import { CanvasPane } from "@/features/canvas/CanvasPane";
import { extractCanvasEntriesFromOutputLines } from "@/features/canvas/canvasParser";
import type { CanvasEntry } from "@/features/canvas/types";
...
const AgentStudioPage = () => {
  ...
  const canvasEntries = useMemo(
    () => {
      const lines = focusedAgent?.outputLines ?? [];
  ...
  const activeCanvasEntry: CanvasEntry | null =
    canvasEntries.length > 0
      ? (canvasEntries[Math.max(0, canvasEntries.length - 1 - canvasDepth)] ?? null)
      : null;
  useEffect(() => {
    setCanvasDepth(0);
  ...
  const sendCanvasPrompt = useCallback(
    (prompt: string) => {
      if (!focusedAgent) return;
      handleSend(focusedAgent.agentId, focusedAgent.sessionKey, prompt);
    },
    [focusedAgent, handleSend]
  );
  ...
  const handleDividerMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      const minPct = (320 / rect.width) * 100;
      setLeftPct(Math.min(Math.max(raw, minPct), 100 - minPct));
    },
    []
  );
        ...
        <div className="flex min-h-screen items-center justify-center px-6">
          <div className="glass-panel ui-panel w-full max-w-md px-6 py-6 text-center">
            ...
            <div className="mt-3 text-sm text-muted-foreground">
              {status === "connecting" ? "Connecting to gateway…" : "Booting Outlet…"}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="relative h-screen w-screen bg-background">
        <div className="relative z-10 flex h-full flex-col">
          <div className="flex shrink-0 items-center gap-2 border-b border-border px-3 py-1.5">
            ...
            <span className="font-mono text-[10px] font-semibold uppercase tracking-[0.08em] text-muted-foreground">Outlet</span>
          </div>
          <div className="flex-1 overflow-y-auto px-3 pb-3 pt-3 sm:px-4 sm:pb-4 sm:pt-4 md:px-6 md:pb-6 md:pt-4">
      ...
      <div className="relative z-10 flex h-screen flex-col">
        <div className="flex min-h-0 flex-1 flex-col">
          {connectionPanelVisible ? (
            <div className="pointer-events-none fixed inset-x-0 top-10 z-[140] flex justify-center px-3 sm:px-4 md:px-5">
          ...
          {settingsRouteActive ? (
            ...
            /* ── Split-pane: left = chat, right = canvas ── */
            <div
              className={`flex min-h-0 flex-1 ${isDragging ? "select-none cursor-col-resize" : ""}`}
            ...
            >
              ...
              <div
                ...
                style={{ width: `${leftPct}%`, minWidth: 320 }}
              >
                {/* Left pane header */}
                <div className="flex h-10 shrink-0 items-center justify-between border-b border-border px-3">
                  <div className="flex items-center gap-2">
                    <span className="text-sm" title="Outlet">🔌</span>
                    <span className="font-mono text-[10px] font-semibold uppercase tracking-[0.08em] text-muted-foreground">Outlet</span>
                    <AgentDropdown
                  ...
                  <div className="relative flex items-center gap-1" ref={settingsMenuRef}>
                    {status === "connecting" ? (
                      <span className="ui-chip px-2 py-0.5 font-mono text-[9px] font-semibold tracking-[0.08em]">
                        Connecting
                      </span>
                    ) : null}
                    <button
                    ...
                    {settingsMenuOpen ? (
                      <div className="ui-card ui-menu-popover absolute right-0 top-8 z-[260] min-w-48 p-1">
                        {focusedAgent ? (
                          <button
                    ...
                    <EmptyStatePanel
                      ...
                      description={
                        hasAnyAgents
                          ? undefined
                          : status === "connected"
                            ? "Use New Agent in the dropdown above to add your first agent."
              ...
              {/* Drag handle */}
              <div
                className="relative w-px shrink-0 cursor-col-resize bg-border transition-colors hover:bg-primary/40 active:bg-primary/60 before:absolute before:inset-y-0 before:-left-px before:w-[3px] before:content-['']"
              ...
              <div
                className="flex min-h-0 flex-col overflow-hidden"
                style={{ width: `${100 - leftPct}%`, minWidth: 320 }}
              >
                <CanvasPane
                  entry={activeCanvasEntry}
                  depth={canvasDepth}
                  totalEntries={canvasEntries.length}
                  onBack={() => setCanvasDepth((d) => d + 1)}
                  onForward={() => setCanvasDepth((d) => Math.max(0, d - 1))}
                  onSendPrompt={sendCanvasPrompt}
                />
              </div>
            </div>
          )}
